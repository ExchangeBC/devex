...
const appendNotificationLink = messagebody => {
		...
		archive.save((err, amessage) => {
			if (err) {
				...
				message.remove(messageErr => {
					if (messageErr) {
						reject(errorHandler.getErrorMessage(messageErr));
					} else {
...
const sendmail = message => {
	...
	return new Promise((resolve, reject) => {
		smtpTransport.sendMail(opts, err => {
			if (err) {
...
const prepareMessage = (template, data) => {
	//
...
const sendMessage = (template, user, data) => {
	data.user = user;
	...
	const message = prepareMessage(template, data);
...
exports.sendMessages = (messageCd, users, data) => {
	if (users.length === 0) {
		return;
	}
	//
...
exports.list = (req, res) => {
	if (!req.user) {
		return sendError(res, 'No user context supplied');
	}
...
exports.myarchivedcount = (req, res) => {
	if (!req.user) {
		return sendError(res, 'No user context supplied');
	}
...
exports.viewed = (req, res) => {
	...
	if (req.user._id.toString() !== req.message.user.toString()) {
		return sendError(res, 'Not owner of message');
	}
...
exports.actioned = (req, res) => {
	...
	// mechanism
	const domain = getDomain();
	const options = getHostInfoFromDomain({
...
// A user purposely archives a given message
exports.archiveold = (req, res) => {
	if (!req.user) {
		return sendError(res, 'No user context supplied');
	}
	if (req.user.roles.indexOf('admin') === -1) {
		return sendError(res, 'Only admin can auto archive');
	}
...
exports.send = (req, res) => {
	if (req.user.roles.indexOf('admin') === -1) {
		return sendError(res, 'Only admin can send via REST');
	}
