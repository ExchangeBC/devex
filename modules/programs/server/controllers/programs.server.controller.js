...
exports.members = function (program, cb) {
	mongoose.model ('User').find ({roles: memberRole(program)}).select ('username displayName updated created roles government profileImageURL email lastName firstName').exec (cb);
};
...
exports.requests = function (program, cb) {
	mongoose.model ('User').find ({roles: requestRole(program)}).select ('username displayName updated created roles government profileImageURL email lastName firstName').exec (cb);
};
