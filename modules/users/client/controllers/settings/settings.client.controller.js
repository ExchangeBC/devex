					...
					user.$update(
						...
						function(response) {
							Notification.error({
								...
								title:
									'<i class="fa fa-3x fa-exclamation-triangle"></i> Edit profile failed!'
							});
						}
					);
				};
					...
					new UsersService(vm.user).$update(
						function(response) {
							...
							Notification.success({
								delay: 2000,
								message:
									'<i class="fa fa-3x fa-check-circle"></i> <h4>Changes saved</h4>'
							});
							CapabilitiesMethods.init(vm, vm.user, capabilities);
						},
						function(response) {
							Notification.error({
								message: response.data.message,
								title:
									'<i class="fa fa-alert-triangle"></i> Changes were not saved!'
							});
						}
					);
				};
