(function () {
  ...
  function PasswordController($scope, $stateParams, UsersService, $location, Authentication, PasswordValidator, Notification) {
    ...
    function onRequestPasswordResetSuccess(response) {
      ...
      vm.credentials = null;
      Notification.success({ message: response.message, title: '<i class="fas fa-ok"></i> Password reset email sent successfully!' });
    }

    function onRequestPasswordResetError(response) {
      // Show user error message and clear form
      vm.credentials = null;
      Notification.error({ message: response.data.message, title: '<i class="fas fa-remove"></i> Failed to send password reset email!', delay: 4000 });
    }
    ...
    function onResetPasswordSuccess(response) {
      ...
      Notification.success({ message: '<i class="fas fa-ok"></i> Password reset successful!' });
      // And redirect to the index page
      $location.path('/password/reset/success');
    }

    function onResetPasswordError(response) {
      Notification.error({ message: response.data.message, title: '<i class="fas fa-remove"></i> Password reset failed!', delay: 4000 });
    }
  }
}());
