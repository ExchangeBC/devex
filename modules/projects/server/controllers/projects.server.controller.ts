...
export class ProjectsController {
	...
	public myadmin = (req, res) => {
		Project.find(this.searchTerm(req, {}))
	...
	public create = (req, res) => {
		...
		//
		Project.findUniqueCode(project.name, null, newcode => {
			project.code = newcode;
			//
			// set the audit fields so we know who did what when
			//
			this.helpers.applyAudit(project, req.user);
			//
			// save and return
			//
			project.save(err => {
	...
	public listMembers = (req, res) => {
		exports.members(req.project, (err, users) => {
			if (err) {
				return res.status(422).send({
					message: this.errorHandler.getErrorMessage(err)
				});
	...
	public listRequests = (req, res) => {
		exports.requests(req.project, (err, users) => {
			if (err) {
				return res.status(422).send({
					message: this.errorHandler.getErrorMessage(err)
				});
	...
	public getProjectForPrograms = (req, res) => {
		Project.find(this.searchTerm(req, { program: req.program._id }))
			.sort('name')
			.populate('createdBy', 'displayName')
			.populate('updatedBy', 'displayName')
			.exec((err, projects) => {
