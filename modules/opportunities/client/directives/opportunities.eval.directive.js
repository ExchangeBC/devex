					...
					function($scope, $state, Authentication, Notification, ProposalsService, ask, modalService) {
							...
							return new Promise(function(resolve, reject) {
								...
								proposals.forEach(function(proposal) {
									...
									if (maxScore > 0) {
										...
										proposal.scores.skill =
											Math.round(
												(proposal.scores.skill / maxScore) *
													(vm.weights.skill * vm.maxPoints) *
													100
											) / 100;
									}
								});
									...
									proposal.scores.total =
										Math.round(
											(proposal.scores.skill +
												...
												proposal.scores.codechallenge +
												proposal.scores.interview +
												proposal.scores.price) *
												100
										) / 100;
								});
							...
							buildQuestionPivot().then(function(values) {
								switch (vm.opportunity.evaluationStage) {
									...
									case vm.stages.pending_review:
										Promise.all([
											randomizeResponseOrder(values.responses),
											calculateSkillScores(values.proposals)
										])
										...
										Math.round(
											(proposal.teamQuestionResponses
												.map(function(response, index) {
													...
													return (
														(vm.proposals.length + 1 - response.rank) *
														teamQuestionWeights[index]
													);
												})
												.reduce(function(a, b) {
													return a + b;
						...
						var calculatePriceScores = function(proposals) {
							...
							passedProposals.forEach(function(proposal) {
								proposal.cost =
									proposal.phases.inception.cost +
									proposal.phases.proto.cost +
									proposal.phases.implementation.cost;
								if (
									lowestBidder !== 0 &&
									(lowestBidder === undefined || proposal.cost < lowestBidder.cost)
								) {
							...
							passedProposals.forEach(function(proposal) {
								if (proposal.cost === 0) {
									...
									proposal.scores.price =
										Math.round(
											(lowestBidder.cost / proposal.cost) *
												(vm.weights.price * vm.maxPoints) *
												100
										) / 100;
								}
							});
								...
								.showModal({
									...
									controller: function($scope, $uibModalInstance) {
										...
										$scope.commit = function() {
											...
											var message =
												'Are you sure you wish to commmit your validation? Ensure you have reviewed all questions.  This action cannot be undone.';
						...
						vm.openQuestionRankingModal = function() {
							...
							if (vm.opportunity.teamQuestionGradingType === 'Linear') {
								modalMarkupUrl =
									'/modules/opportunities/client/views/swu-opportunity-modal-eval-questions-linear.html';
							} else {
								modalMarkupUrl =
									'/modules/opportunities/client/views/swu-opportunity-modal-eval-questions-weighted.html';
							}
										...
										resp.proposalScores.forEach(function(score) {
											...
											if (match) {
												if (score.score / vm.maxCodeChallengePoints >= 0.8) {
													...
													match.scores.codechallenge =
														Math.round(
															(score.score / vm.maxCodeChallengePoints) *
																(vm.weights.codechallenge * vm.maxPoints) *
																100
														) / 100;
													passCount++;
												} else {
								...
								.then(function(resp) {
									if (resp.action === 'save') {
										...
										resp.proposalScores.forEach(function(score) {
											...
											if (match) {
												match.scores.interview =
													Math.round(
														(score.score / vm.maxInterviewPoints) *
															(vm.weights.interview * vm.maxPoints) *
															100
													) / 100;
												scoreCount++;
											}
						...
						vm.resetEvaluation = function() {
							var message =
								'WARNING: This will reset the current evaluation and any calculations or entered data will be lost.  Proceed?';
							ask.yesNo(message).then(function(response) {
								if (response) {
						...
						vm.undoLastStage = function() {
							var message =
								'WARNING: This will cause any calculations or entered data from the last stage to be erased.  Proceed?';
							ask.yesNo(message).then(function(response) {
								if (response) {
									if (
										vm.opportunity.evaluationStage === vm.stages.price ||
