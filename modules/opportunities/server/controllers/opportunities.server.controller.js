...
var oppBody = function (opp) {
	...
	var ret = '';
	ret += 'Value: '+earn'<br>';
	ret += 'Closes: '+deadline'<br>';
	ret += 'Location: '+opp.location+' '+locs[opp.onsite]'<br>';
	ret += '<h3>Opportunity Description</h3>';
	ret += opp.description;
	ret += '<h3>Acceptance Criteria</h3>';
	ret += opp.criteria;
	ret += '<h3>How to Apply</h3>';
	ret += '<p>Visit the <a href="https://bcdevexchange.org/opportunities/'+opp.code+'">Opportunity Page</a> to submit a proposal. Deadline for proposals is <b>16:00 Pacific Time</b> on <b>'+deadline+'</b>.</p>';
	...
	ret += '<h3>Proposal Evaluation Criteria</h3>';
	ret += opp.evaluation;
	return ret;
};
// -------------------------------------------------------------------------
