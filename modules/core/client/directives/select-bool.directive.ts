...
(() => {
	'use strict';
	angular.module('core').directive('selectBoolean', () => {
		interface ICompileScope extends ng.IScope {
		...
		return {
			...
			template(elem, attrs) {
				...
				tmpl +=
					'select-bool-control' +
					(options.stacked ? ' select-bool-stacked' : '') +
					'"';
				...
				tmpl +=
					...
					'" name="' +
					options.name +
					'" ng-click="toggleValue($event)" class="btn btn-full-left ' +
					(options.icon ? ' btn-has-icon' : '') +
					"\" ng-class=\"{'btn-default':!ngModel, 'btn-{{ " +
					options.trueClass +
					"|| 'info' }}': ngModel}\">";
				if (options.title) {
				...
				if (options.icon) {
					tmpl +=
						'<span ng-if="ngModel" class="fas fa-check-circle guttered" aria-hidden="true"> &nbsp;</span>';
					tmpl +=
						'<span ng-if="!ngModel" class="fas fa-exclamation-triangle guttered" aria-hidden="true"> &nbsp;</span>';
				}
