(() => {
	...
	function showErrors($timeout, $interpolate) {
		...
		function compile(elem, attrs) {
			if (
				attrs.showErrors.indexOf('skipFormGroupCheck') === -1 &&
				!(elem.hasClass('form-group') || elem.hasClass('input-group'))
			) {
				throw new Error(
					"show-errors element does not have the 'form-group' or 'input-group' class"
				);
			}

			return linkFn;

			function linkFn(scope, el, linkAttrs, formCtrl) {
				let inputEl;
				let inputName;
				let inputNgEl;
				let options;
				...
				options = scope.$eval(linkAttrs.showErrors) || {};
				...
				if (!inputName) {
					throw new Error(
						"show-errors element has no child input elements with a 'name' attribute class"
					);
				}
