...
(function() {
	...
	gobal.keyHelper = {
		...
		floatKeyBoard: function(event, viewValue) {
			...
			return (
				[188].indexOf(which) !== -1 ||
				((which === 190 || which === 110) && viewValue.toString().indexOf('.') === -1)
			);
		}
	};
		...
		.constant('modelFormatConfig', {
			currency: {
				...
				keyDown: function(args) {
					...
					if (
						!(
							gobal.keyHelper.smallKeyBoard(event) ||
							gobal.keyHelper.numberKeyBpoard(event) ||
							gobal.keyHelper.functionKeyBoard(event) ||
							gobal.keyHelper.currencyKeyBoard(event, viewValue) ||
							gobal.keyHelper.floatKeyBoard(event, viewValue)
						)
					) {
