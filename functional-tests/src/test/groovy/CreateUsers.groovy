...
@Narrative('''This test creates three users:
    ...
    HibouleBlanc
    HectorCunniculus
the three of them have already been set up in GitHub. For the three of them ,the pwd is DEVEX_Test1
It also configures their settings and claims the Cooking capability. This Capability needs to have been
previously created via the CreateCapability test

These users will be used in other tests.

The test finishes with each user logged off from DevExchange and GitHub
 ''')
...
class CreateUsers extends GebReportingSpec {
  ...
  def "Go to Home Page and click on the log to GitHub button as user '#UserFirstName' '#UserLastName'"() {
    ...
    and: "Arrive to the GitHub login page, where we will be able to log using the credentials '#Login' and '#Pwd'"
        ...
        sleep(2000) // Leave time in case the next page is the reauthorization page
    ...
    then: "After successful Login, arrive at the Profile page for the user"
        to SingleProfilePage  // Ensure we are in the user's settings page
