	...
	return clearTemplates ().then (function () {
		return Promise.all ([
			new T ({
				...
				emailBodyTemplate    : '<p>Hi {{user.displayName}},</p><p><strong>You have been invited you to join <strong>{{org.name}}</strong> so they can put you forward as a team member on proposals for <i>Sprint With Us</i> opportunities.</p>',
			...
			new T ({
				...
				messageShortTemplate : '',
				messageTitleTemplate : 'You\'ve been invited to join {{org.name}}',
				emailBodyTemplate    : '<p>Hi {{user.displayName}}</p>Company {{org.name}} is inviting you to sign up on the BCDevExchange and join <strong>{{org.name}}</strong> so they can put you forward as a team member on a proposal.</p>',
			...
			new T ({
				...
				emailBodyTemplate    : '<img src="https://bcdevexchange.org/modules/core/client/img/logo/new-logo-220px.png"> <br/><br/> Hi {{user.displayName}}, <br/><br/> A new opportunity has just been posted: <h4>{{ opportunity.name }}</h4> <ul><li>Value: <b>{{ opportunity.earn }}</b></li><li>Value: <b>{{ opportunity.deadline_format_date }}</b></li></ul><h4><a href="{{ domain }}/{{ opportunity.path }}">Click here to see the details</a></h4> Have a great day!<br/><b>The BCDevExchange Team</b><br/><br/>--- <i>To stop receiving notifications about new opportunities, <a href="{{ domain }}/opportunities">browse to the opportunity listing</a> and stop listening, or visit <a href="{{ domain }}/settings/privacy">your profile</a> and uncheck "Tell me about new opportunities" </i>',
