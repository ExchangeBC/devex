	...
	return clearTemplates ().then (function () {
		return Promise.all ([
			new T ({
				...
				emailBodyTemplate    : '<p>Hi {{user.displayName}},</p><p>You\'ve been invited to join <strong>{{org.name}}</strong>. If you accept, {{org.name}} will be able to put you forward as a team member on proposals for <i>Sprint With Us</i> opportunities.</p>',
			...
			new T ({
				...
				messageShortTemplate : '',
				messageTitleTemplate : 'You\'ve been invited to join {{org.name}}',
				emailBodyTemplate    : '<p>Hi {{user.displayName}}</p><p>You\'ve been invited to sign up on the BCDevExchange and to join <strong>{{org.name}}</strong>.</p>',
